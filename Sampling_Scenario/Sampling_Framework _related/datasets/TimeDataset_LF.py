...
class TimeDataset(Dataset):
    def __init__(self, raw_data, edge_index, config=None, mode='train'):
    ...
    def process(self, data, labels):
        ...
        if self.mode == 'train':
            ...
            mrr_sample = torch.tensor([
                mrr_all[i] if i % 2 == 0 else mrr_all[i-1]
                for i in range(len(mrr_all))
            ])
        ...
        for i in rang2:
            ...
            mrr = mrr_sample[int(i / 316) - past_wafer:int(i / 316)]  # retain only one MRR for a wafer
            mrr_arr.append(mrr)
